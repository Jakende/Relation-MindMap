...
function initializeForceGraph(data) {
  ...
  link.on("mouseout", function(event, l) {
    if (window.showRelationLabels) {
      linkLabels.style("opacity", 0);
    }
  });
  // === Relations-Only Toggle ===
  if (window.showRelationsOnly) {
    link.style("display", l => l.type && l.type !== 'hierarchy' ? null : 'none');
    linkLabels.style("display", l => l.type && l.type !== 'hierarchy' ? null : 'none');
  } else {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialisieren
  loadCurrentData();
  // Relations-Only Toggle-Button einfügen (nur einmal)
  if (!document.getElementById('relations-toggle')) {
    const relationsToggle = document.createElement('button');
    relationsToggle.id = 'relations-toggle';
    relationsToggle.title = 'Relations-Beschriftungen anzeigen/ausblenden';
    relationsToggle.setAttribute('aria-label', 'Relations-Beschriftungen anzeigen/ausblenden');
    relationsToggle.style.position = 'fixed';
    relationsToggle.style.right = '24px';
    relationsToggle.style.bottom = '156px'; // direkt unter showall-toggle
    relationsToggle.style.width = '44px';
    relationsToggle.style.height = '44px';
    relationsToggle.style.border = 'none';
    relationsToggle.style.borderRadius = '50%';
    relationsToggle.style.background = 'rgba(0,0,0,0.08)';
    relationsToggle.style.color = 'var(--palette-primary)';
    relationsToggle.style.fontSize = '22px';
    relationsToggle.style.cursor = 'pointer';
    relationsToggle.style.boxShadow = '0 2px 8px rgba(0,0,0,0.10)';
    relationsToggle.style.backdropFilter = 'blur(8px)';
    relationsToggle.innerText = '🔗';
    document.body.appendChild(relationsToggle);
    window.showRelationLabels = false;
    relationsToggle.addEventListener('click', () => {
      window.showRelationLabels = !window.showRelationLabels;
      if (currentLayout === 'graph') {
        initializeForceGraph(currentGraph);
      }
      relationsToggle.style.background = window.showRelationLabels ? 'var(--palette-primary)' : 'rgba(0,0,0,0.08)';
      relationsToggle.style.color = window.showRelationLabels ? '#fff' : 'var(--palette-primary)';
    });
  }

  // Palette- und Darkmode-Button nach oben verschieben
  document.getElementById('palette-toggle').style.bottom = '248px';
  document.getElementById('darkmode-toggle').style.bottom = '294px';
});
